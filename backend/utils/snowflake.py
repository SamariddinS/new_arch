...
class Snowflake:
    ...
    def __init__(
        ...
        if cluster_id < 0 or cluster_id > SnowflakeConfig.MAX_DATACENTER_ID:
            raise errors.RequestError(msg=f'Cluster ID must be between 0-{SnowflakeConfig.MAX_DATACENTER_ID}')
        if node_id < 0 or node_id > SnowflakeConfig.MAX_WORKER_ID:
            raise errors.RequestError(msg=f'Node ID must be between 0-{SnowflakeConfig.MAX_WORKER_ID}')
    ...
    def _next_millis(self, last_timestamp: int) -> int:
        ...
        :param last_timestamp: Timestamp of last ID generation
        :return:
        """
    ...
    def parse_id(snowflake_id: int) -> SnowflakeInfo:
        """
        Parse snowflake ID to get detailed information
