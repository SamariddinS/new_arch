...
async def rbac_verify(request: Request, _token: str = DependsJwtAuth) -> None:  # noqa: C901
    """
    RBAC permission verification (authentication order is important, modify carefully)
    ...
    # Force JWT authorization status verification
    ...
    if not user_roles or all(status == 0 for status in user_roles):
        raise errors.AuthorizationError(msg='User has not been assigned roles, please contact system administrator')

    # Check user role menus
    if not any(len(role.menus) > 0 for role in user_roles):
        raise errors.AuthorizationError(msg='User has not been assigned menus, please contact system administrator')

    # Check backend management operation permission
    method = request.method
    if (method != MethodType.GET or method != MethodType.OPTIONS) and not request.user.is_staff:
        raise errors.AuthorizationError(msg='User has been prohibited from backend management operations, please contact system administrator')

    # RBAC authentication
    if settings.RBAC_ROLE_MENU_MODE:
        path_auth_perm = ctx.permission

        # No verification if there is no menu operation permission identifier
        ...
        # Verify assigned menu permissions
        allow_perms = []
        ...
        try:
            ...
            log.error(f'Executing RBAC permission verification through casbin, but plugin does not exist: {e}')
            raise errors.ServerError(msg='Permission verification failed, please contact system administrator')
